...
class RedistributionSpecs extends Specification {

  val initial = Redistribution(Vector(0, 2, 7, 0))
  val firstStep = Redistribution(initial.step)
  val secondStep = Redistribution(firstStep.step)
  val thirdStep = Redistribution(secondStep.step)
  val fourthStep = Redistribution(thirdStep.step)
  val fifthStep = Redistribution(fourthStep.step)
...
- Now, there is a tie between the first and fourth memory banks, both of which have three blocks.
  ...
  ${thirdStep.mem === Vector(0, 2, 3, 4)}
- The fourth bank is chosen, and its four blocks are distributed such that each of the four banks receives one: 1 3 4 1.
  ${fourthStep.mem === Vector(1, 3, 4, 1)}
- The third bank is chosen, and the same thing happens: 2 4 1 2.
  ${fifthStep.mem === Vector(2, 4, 1, 2)}
