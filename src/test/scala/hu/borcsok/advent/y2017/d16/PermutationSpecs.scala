...
class PermutationSpecs extends Specification {

  val input: String = readResource("2017/permutation.txt")
...
- Spin, written sX, makes X programs move from the end to the front, but maintain their order otherwise.
  ...
  ${Permutation.run("s3", 5) === "cdeab"}
- Exchange, written xA/B, makes the programs at positions A and B swap places.
  ${Permutation.run("x2/4", 5) === "abedc"}
- Partner, written pA/B, makes the programs named A and B swap places.
  ${Permutation.run("pa/c", 5) === "cbade"}
...
${Permutation.run("s1,x3/4,pe/b", 5) === "baedc"}
...
${Permutation.run(input, 16) === "iabmedjhclofgknp"}
...
${Permutation.stress(input, 16) === "oildcmfeajhbpngk"}
