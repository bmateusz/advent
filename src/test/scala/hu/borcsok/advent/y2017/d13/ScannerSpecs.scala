...
class ScannerSpecs extends Specification {

  val input: Seq[String] = readResourceLines("2017/scanner.txt")
...
and moves down until it reaches the bottom, then moves up until it reaches the top, and repeats.
...
you are caught. (If a scanner moves into the top of its layer while you are there, you are not caught:
...
${ScannerHelper.fromStrings(example).simulate === Seq(0, 6)}
${ScannerHelper.fromStrings(example).severity === 24}

Given the details of the firewall you've recorded, if you leave immediately, what is the severity of your whole trip?

${ScannerHelper.fromStrings(input).severity === 2508}
...
${ScannerHelper.fromStrings(example).safelyAndSlowly() === 10}
${ScannerHelper.fromStrings(example).safely === 10}

What is the fewest number of picoseconds that you need to delay the packet to pass through the firewall without being caught?
${ScannerHelper.fromStrings(input).safely === 3913186}
"""
