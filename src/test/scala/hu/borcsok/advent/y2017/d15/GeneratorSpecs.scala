...
class GeneratorSpecs extends Specification {
  ...
  val pickyInput: Generator = input.toPicky

  val fortyMillion: Int = 40000000

  val fiveMillion: Int = 5000000
...
only occasionally handing an acceptable value to the judge, and otherwise working through the same sequence of
...
the first values from each generator are compared, then the second values from each generator, then the third values, and so on.
...
it is now only willing to consider 5 million pairs. (Using the values from the example above, after five million pairs,
the judge would eventually find a total of 309 pairs that match in their lowest 16 bits.)

${pickyExample.judge(fiveMillion) === 309}
